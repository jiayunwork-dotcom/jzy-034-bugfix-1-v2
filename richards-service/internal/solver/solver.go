...
// gamma[0]: state-dependent surface conductance for a ponded top
// (harmonic mean of surface Ks and cell-0 K over the half-cell distance),
// assembled by topConductance; 0 for a zero-flux top.
...
// mean is essential: when the top cell is nearly dry K0 << Ks it tends to
// 2*K0/dz, so the dry soil itself throttles infiltration; an arithmetic mean
// would stay near Ks and let water bypass the low-conductivity top layer.
func (s *Solver) topConductance(k0 float64) float64 {
	return 2.0 * s.mean.mean(s.Params.Ks, k0) / s.Grid.Dz
}

// dTopConductanceDk0 is d(topConductance)/dK0, used by the Newton Jacobian.
func (s *Solver) dTopConductanceDk0(k0 float64) float64 {
	return 2.0 * s.mean.dMeanDDown(s.Params.Ks, k0) / s.Grid.Dz
}
...
func (s *Solver) Step(dt float64) (*StepResult, error) {
	...
	for iter = 0; iter < maxIter; iter++ {
		...
		switch s.TopKind {
		case TopPondedHead:
			// q0 = gamma0 * (hP - h0 + dz/2), with
			// gamma0 = (2/dz) * mean(Ks, K(h0)) the state-dependent
			// surface conductance; -q0 enters row 0.
			factor0 := s.PondedH - hIt[0] + dz/2.0
			dq0dn := s.dTopConductanceDk0(kCell[0])*dkCell[0]*factor0 -
				s.topConductance(kCell[0])
			diag[0] += -dq0dn
		case TopZeroFlux:
			// q0 = 0
		}
