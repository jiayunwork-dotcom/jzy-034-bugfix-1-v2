...
- 相邻层导水率：**调和平均** `Kf = 2 Ki Kj/(Ki+Kj)`，绝不使用算术平均（算术平均
  ...
  的对照用例）。
- 上边界：`ponded_head`（给定表面积水水头，Dirichlet）或 `zero_flux`。积水边界
  的有效过流能力同样取**调和平均**：表面积水面（K = Ks）与土柱顶层 K(h0) 在半
  层距 dz/2 上合成 `2*Kf/dz`，因此初始很干燥的顶层（K << Ks）会明显卡住入流，
  而不会被绕过。
...
可区分、积水顶边界对干燥顶层的入流阻塞（调和平均，干柱首步通量远低于近饱和柱
且与手算一致）、五类非法参数分别被挡、单步/整段同初值同结果、并发多作业互不
串扰。
